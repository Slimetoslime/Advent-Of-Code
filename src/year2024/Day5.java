...
public class Day5 {
	 ...
	 * 
	 * @param rulesString A list of Strings containing the rules for the available page numbers.
	 */
	...
	/** Custom comparator object that compares 2 Strings with the HashMap rules listed above.
	 * 
	 */
	static class sortOnRules implements Comparator<String> {
		@Override
		public int compare(String s1, String s2) {
			if (rules.containsKey(s1) && rules.get(s1).contains(s2)) return -1;
			else if (reversedRules.containsKey(s2) && reversedRules.get(s2).contains(s1)) return 1;
			return 0;
		}
	}
	...
	public static boolean checkForValidity(List<String> pageNumbers) {
		...
		while (currentPage < pageNumbers.size()) {
			...
			for (int i = currentPage+1; i < pageNumbers.size(); i++) {
				try {
					if (!listRules.contains(pageNumbers.get(i))) valid = false;
			...
			for (int i = reversedCurrentPage-1; i > -1; i--) {
				try {
					if (!listReversedRules.contains(pageNumbers.get(i))) valid = false;
				} catch (NullPointerException npe) {
					continue;
				}
			}
